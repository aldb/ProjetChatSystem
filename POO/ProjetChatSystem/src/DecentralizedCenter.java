...
public class DecentralizedCenter extends AbstractModel implements Runnable
{
	...
	public void run()
	{
        while (true)
        {
            ...
            if(notification != null && this.usernameStatus != UsernameStatus.ALREADY_TAKEN_OR_NOT_ASSIGNED) // && notification.getIpAddress() != currentUser.getIpAddress() // à tester/changer equals/toString
            {
                ...
                else if (notification.getData().charAt(0)=='c')
                {
                    ...
                    if (false) // TODO: for local test, must be :(userListModel.find(sender) != null || sender.equals(currentUser)) // username existant dans la liste
                    {
